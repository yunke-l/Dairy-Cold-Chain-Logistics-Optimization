# -*- coding: utf-8 -*-
"""
Created on Thu Jul  9 11:46:55 2020
...
# Fixed Parameters
C_dc=[12318, 11308, 8938, 14703, 9073]
price = 3000            # Yuan per quantity
C_k = 1000              # truck fixed cost Yuan
k = 20                  # No. of trucks
L = 5                   # No. of DC
nPop = 50               # No. of customers, initial population
mileage_0 = 8           # km per Litre
mileage_m = 6           # km per Litre
load_capacity = 150     # kg for the truck
load_truck = 2000       # kg
load_max = load_truck + load_capacity
fuel = 70               # Yuan per Litre
speed = 0.4             # Km per min
t_unload = 15           # min
unit_mass = 10          # kg
mu_1 = 300              # Yuan per hour
mu_2 = 300              # Yuan per hour
delta_1 = 0.002         # Deterioration rate in transit
delta_2 = 0.003         # Deterioration rate when serving a customer
Q_dc = 350              # kg for DC
RefCost_t = 15          # Yuan per hour
RefCost_u = 20          # Yuan per hour
Carbon_dc = 200         # Fixed Carbon Cost Yuan
C_fuel = 0.4            # Carbon Cost Yuan per kg
p = 11.25               # Power of refrigerator kW
t_ref = 24              # Average operating time of refrigerator hour
E = 0.997               # Proportion of electricity transfer to carbon emissions kg/(kW*h)
...
# Mileage function
def mileage(load):
    consumption = mileage_0 + ((mileage_m - mileage_0) * (load - load_truck)) / (load_capacity)
    return consumption
...
def cost_func(S1, S2, S3, Carbon):
    ...
    # Refrigeration Cost C3
    # Penalty Cost C4
    # Damage Cost C5
    # Carbon Emission Cost (given in order) C6
    ...
    for i in range(0, L):
        ...
        C1 += Z[i]*(fixed_dc + fixed_truck)
        C61 += Z[i]*p*t_ref*E*C_fuel  #Distribution center
...
result = []
for C in range(1):
    best, logs = GeneticAlgo(500, C, 50)
